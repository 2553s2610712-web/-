...
def check_if_dessert(food_name):
    try:
        ...
        # API 호출 중 에러가 발생하면 앱이 멈추지 않도록 안전하게 True를 반환하여 등록 허용
        return True
...
if not st.session_state.dessert_list:
    ...
    for item in st.session_state.dessert_list:
        # 깔끔한 카드 형태로 출력하기 위한 HTML/CSS 스타일 적용
        st.markdown(
            f"""
            <div style="background-color: #f9f9f9; padding: 15px; border-radius: 10px; margin-bottom: 12px; border-left: 5px solid #ff4b4b; shadow: 1px 1px 3px rgba(0,0,0,0.1);">
